/* =========================================================================
   ...
   =========================================================================
   1) Usa "hosted-fields.dev.malga.io" para sandbox.
   2) Apresenta steps (1..4), fluxo de cartões, Pix, Boleto.
   3) Tem máscaras (CPF, RG), busca CEP, e modal de passageiros extras.
   4) Ajuste as partes marcadas (API Key / Client ID / nome do endpoint).
   ========================================================================= */

import * as Malga from '@malga/tokenization';

/* =========================================================================
   0) Suas credenciais sandbox. Ajuste conforme a Malga forneceu.
   ========================================================================= */
...
/* =========================================================================
   1) Criar instância do Tokenization no modo sandbox
   ========================================================================= */
const malgaTokenization = new Malga.MalgaTokenization({
  apiKey: MALGA_API_KEY,
  clientId: MALGA_CLIENT_ID,
  options: {
    // allowedOrigins pode ser "*", mas ideal é pôr o domínio exato do seu site:
    allowedOrigins: ["*"], 
    config: {
      fields: {
        cardNumber: {
      ...
      sandbox: true, // <--- fundamental para Hosted Fields .dev.malga.io
      styles: {
        input: {
          color: "black",
          "font-size": "14px"
...
// Algumas versões do SDK pedem malgaTokenization.init(); 
// Se "init" existir, descomente:
// if (typeof malgaTokenization.init === "function") {
//   malgaTokenization.init();
// }

// Para fins de debug, capturamos erros detalhados do SDK:
...
/* =========================================================================
   3) processCardPayment - envia tokenId do cartão p/ /api/malga/create-transaction
   ========================================================================= */
async function processCardPayment(tokenId) {
  try {
    const amount = getOrderAmount();
    // Se tiver select de parcelas, capture aqui. Exemplo fixo "1":
    ...
    if (result.success) {
      ...
      showStep(4); // Ir para step 4 => Confirmação
    } else {
...
/* =========================================================================
   4) Inicializa Hosted Fields (cartão). Ao dar submit no form, faz tokenize().
   ========================================================================= */
function initializeCardForm() {
  ...
  form.addEventListener("submit", async (evt) => {
    evt.preventDefault();
    try {
      console.log("SDK chegou aqui no submit", document.querySelector("iframe[name=card-number]"), malgaTokenization.configurations);
      ...
      // Sucesso => chamar processCardPayment
...
function initializePix() {
  ...
  btn.addEventListener("click", async () => {
    try {
      ...
      alert("Erro ao gerar Pix. Ver console p/ detalhes.");
    }
  });
}

/* =========================================================================
   6) initializeBoleto - gera link/linha digitável chamando /api/malga/...
   ========================================================================= */
function initializeBoleto() {
  ...
  btn.addEventListener("click", async () => {
    try {
      ...
      alert("Erro ao gerar Boleto. Ver console p/ detalhes.");
    }
  });
}

/* =========================================================================
   7) switchPaymentMethod => exibe form cartão ou containers Pix/Boleto
   ========================================================================= */
...
// Objeto "checkoutData" p/ guardar info de extraPassengers, etc.
const checkoutData = {
  extraPassengers: [],
...
/* =========================================================================
   10) updateCheckoutCart => exibir carrinho na coluna direita
   ========================================================================= */
...
/* =========================================================================
   11) Modal de Passageiros Extras (exemplo)
   ========================================================================= */
...
if (toStep2Btn) {
  toStep2Btn.addEventListener("click", () => {
    // Exemplo: checar se user está logado ou se preencheu os campos
...
window.addEventListener("load", () => {
  ...
  // Se user já logado, esconde link login
  const isLoggedIn = !!localStorage.getItem("agentId");
  if (isLoggedIn) {

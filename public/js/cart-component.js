...
class ShoppingCart extends HTMLElement {
  constructor() {
    ...
    // Array para armazenar os itens do carrinho (usados ao adicionar itens, por exemplo, de quartos)
    this.items = [];
    this.shareId = null;
    
    // Dados padrão do item (para exibição de exemplo, podem ser sobrescritos pelos atributos)
    this.item = {
    ...
    this.shadowRoot.innerHTML = `
      ...
      <div class="cart-container">
        ...
        <div class="cart-body" id="cartBody">
          ...
          <!-- ITEM EXEMPLO (usando os valores padrão do atributo item) -->
          <div class="cart-item">
  ...
  connectedCallback() {
    ...
    // Adiciona event listeners para os controles de quantidade, se existirem no template
    const incChild = this.shadowRoot.querySelector('.inc-children');
    if (incChild) incChild.addEventListener('click', () => this.incChildren());
    const decChild = this.shadowRoot.querySelector('.dec-children');
    if (decChild) decChild.addEventListener('click', () => this.decChildren());
    const incAdult = this.shadowRoot.querySelector('.inc-adults');
    if (incAdult) incAdult.addEventListener('click', () => this.incAdults());
    const decAdult = this.shadowRoot.querySelector('.dec-adults');
    if (decAdult) decAdult.addEventListener('click', () => this.decAdults());

    // Verifica se há shareId salvo e carrega do servidor
  ...
  // Atualiza a exibição do item padrão (opcional, apenas para exibição no template)
  renderItem() {
